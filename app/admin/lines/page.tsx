...
export default function AdminLinesPage() {
  ...
  async function save(e:FormEvent){
    ...
    try {
      ...
      const values={...form,title:form.title.trim(),body:form.body.trim(),direction:form.direction.trim()||null,category:form.category.trim()||null,image_url,image_storage_path,published_at:form.status==="published"?(editing?.published_at??new Date().toISOString()):null,updated_at:new Date().toISOString()};
  ...
  function edit(row:Row){setEditing(row);setForm({title:row.title,body:row.body,direction:row.direction??"",category:row.category??"",status:row.status});setImage(null)}
  ...
  return <main className="min-h-screen bg-neutral-100 px-4 py-10"><div className="mx-auto max-w-6xl"><p className="text-sm font-bold text-blue-600">KOELABO</p><h1 className="mt-2 text-3xl font-black">セリフ管理</h1>{message&&<p className="mt-4 rounded-xl bg-white p-4 text-sm">{message}</p>}<div className="mt-8 grid gap-6 lg:grid-cols-[420px_1fr]">
    ...
    <div className="space-y-4">{rows.map(row=><article key={row.id} className="rounded-2xl bg-white p-5 shadow-sm"><div className="flex gap-4">{row.image_url?<img src={row.image_url} alt="" className="h-24 w-32 rounded-xl object-cover"/>:<div className="h-24 w-32 rounded-xl bg-blue-50"/>}<div className="min-w-0 flex-1"><span className="text-xs font-bold text-blue-700">{row.status==="published"?"公開":"下書き"}</span><h2 className="mt-2 font-bold">{row.title}</h2><p className="line-clamp-2 whitespace-pre-wrap text-sm text-neutral-600">{row.body}</p></div></div><div className="mt-4 flex flex-wrap gap-2"><label className="cursor-pointer rounded-xl bg-violet-50 px-4 py-2 text-sm font-bold text-violet-700">{replacingId===row.id?"差し替え中…":"画像だけ差し替え"}<input disabled={Boolean(replacingId)} type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="sr-only" onChange={e=>{const file=e.target.files?.[0];if(file)void replaceImage(row,file);e.target.value=""}}/></label><button onClick={()=>edit(row)} className="rounded-xl bg-blue-50 px-4 py-2 text-sm font-bold text-blue-700">内容を編集</button><button onClick={()=>remove(row)} className="rounded-xl bg-red-50 px-4 py-2 text-sm font-bold text-red-700">削除</button></div></article>)}</div>
  </div></div></main>;
}
